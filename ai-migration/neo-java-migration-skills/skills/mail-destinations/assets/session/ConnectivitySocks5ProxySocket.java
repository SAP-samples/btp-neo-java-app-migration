...
public class ConnectivitySocks5ProxySocket extends Socket {
    ...
    public void connect(SocketAddress endpoint, int timeout) throws IOException {
        // Resolve the connectivity service binding once per connect — both the proxy
        // address and the JWT token are derived from the same credentials map, so
        // looking it up twice would be wasted work.
     ...
     * This avoids pulling in a third-party JSON library — a previous version of this
     * class used {@code org.json}, which the SDK BOM manages at {@code provided} scope,
     * causing {@code NoClassDefFoundError} at runtime once the WAR was deployed.</p>
     */
